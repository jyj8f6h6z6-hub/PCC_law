...
function selectArticle(a){let law=$('#articleLaw').value,ar=findArticle(law,a);if(!ar)return;document.querySelectorAll('.article-row').forEach(x=>x.classList.toggle('selected',x.dataset.a===a));let rel=lettersFor(law,a).sort((x,y)=>y.index-x.index),exactIds=new Set(EXACT_INDEX.get(law+'|'+a)||[]),exact=rel.filter(r=>exactIds.has(r.index)),mention=rel.filter(r=>!exactIds.has(r.index)),relatedLaws=law==='政府採購法'?lawsForAct(a):[],relatedRules=law==='政府採購法'?relatedRulesForAct(a):[];let c=[...document.querySelectorAll('.article-count')].find(x=>x.dataset.countLaw===law&&x.dataset.countA===a);if(c)c.textContent=exact.length?exact.length+'筆精準／'+rel.length+'筆相關':rel.length+'筆相關';let links=arr=>arr.slice(0,120).map(r=>`<a href="#" class="related-link gotoletter" data-id="${r.index}">#${String(r.index).padStart(4,'0')}｜${esc(r.rocDate)}｜${esc(r.docNo)}<small>${esc(r.subject||'')}</small></a>`).join('');let rulesHtml=relatedRules.length?`<div class="related-section rules-inline"><h3>相關施行細則 <span class="muted">${relatedRules.length}條</span></h3>${relatedRules.map(x=>`<div class="inline-rule"><h4>政府採購法施行細則 ${esc(x.no)}</h4><div class="inline-rule-text">${linkRefs(x.text)}</div></div>`).join('')}<div class="relation-note">本法與施行細則對照依人工確認對照表顯示。</div></div>`:'';let lawLinks=relatedLaws.length?`<div class="related-section"><h3>引用／關聯主管法規 <span class="muted">${relatedLaws.length}筆</span></h3>${relatedLaws.slice(0,80).map(l=>`<a href="#" class="related-link openlaw" data-lawname="${esc(l.name)}"><b>${esc(l.name)}</b><small>${esc(l.category)}｜${esc(l.updated||'')}</small></a>`).join('')}</div>`:'';$('#articleDetail').innerHTML=`<h2>${esc(law)} ${esc(ar.no)}</h2><div class="article-text">${linkRefs(ar.text)}</div>${rulesHtml}${lawLinks}<div class="related-section"><h3>PRMS原始法規依據－精準關聯 <span class="muted">${exact.length}筆</span></h3>${exact.length?links(exact):'<div class="empty">PRMS原始「根據」欄位目前無直接關聯</div>'}</div><div class="related-section"><h3>函釋全文提及－延伸關聯 <span class="muted">${mention.length}筆</span></h3>${mention.length?links(mention):'<div class="empty">目前無其他全文提及</div>'}</div>`}
