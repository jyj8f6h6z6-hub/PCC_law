...
function runSearch(){
  ...
  $('#lawHits').innerHTML=lh.length?`<section class="lawhits-group expanded"><button type="button" class="lawhits-toggle" aria-expanded="true"><span>主管法規找到 <b>${lh.length.toLocaleString()}</b> 個命中位置，共 <b>${lawGroups.length.toLocaleString()}</b> 部法規</span><span class="lawhits-toggle-label">▲ 全部收合</span></button><div class="lawhits-body">${lawItems}</div></section>`:'';
