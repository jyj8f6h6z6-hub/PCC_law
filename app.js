...
function runSearch(){
  ...
  const lawExpanded=scope==='laws';
  const lawItems=lh.slice(0,200).map(h=>`<button class="lawhit lawhit-point" data-lawname="${esc(h.law.name)}" data-target="${h.target}" data-targetkey="${esc(h.targetKey)}" data-search="${esc($('#q').value)}"><b>${esc(h.law.name)}｜${esc(h.no)}</b><span class="hit-snippet">${snippet(h.text,ts)}</span></button>`).join('')+(lh.length>200?`<div class="status">結果較多，目前顯示前200個命中位置。</div>`:'');
  $('#lawHits').innerHTML=lh.length?`<section class="lawhits-group ${lawExpanded?'expanded':'collapsed'}"><button type="button" class="lawhits-toggle" aria-expanded="${lawExpanded}"><span>主管法規找到 <b>${lh.length.toLocaleString()}</b> 個命中位置</span><span class="lawhits-toggle-label">${lawExpanded?'▲ 收合':'▼ 展開'}</span></button><div class="lawhits-body" ${lawExpanded?'':'hidden'}>${lawItems}</div></section>`:'';
  $('#letterHits').innerHTML='';renderSearchMore();$('#searchStatus').className='status';$('#searchStatus').textContent=ts.length?`搜尋「${$('#q').value}」`:'請輸入關鍵字搜尋145筆主管法規與3,666筆函釋';
}
